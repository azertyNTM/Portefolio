...
export function CaseNarrative() {
  ...
  return <section className="case-narrative" aria-label="Le déroulé du projet Speed-to-Lead"><div className="narrative-art" aria-hidden="true"><div className={`narrative-sheet sheet-${active}`}><span>Speed-to-Lead / {String(active + 1).padStart(2, "0")}</span><strong>{steps[active].label}</strong><i>{active === 0 ? "une intention" : active === 1 ? "une histoire" : "une prochaine action"}</i></div></div><div className="narrative-steps">{steps.map((step, index) => <article key={step.title} data-step={index} ref={(element) => { refs.current[index] = element; }} className={active === index ? "is-active" : ""}><span>{String(index + 1).padStart(2, "0")}</span><h3>{step.title}</h3><p>{step.copy}</p></article>)}</div></section>;
}

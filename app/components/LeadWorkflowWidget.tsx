...
function DetectedWorkflow({ revealed }: { revealed: number }) {
  return <div className="workflow-body" key="detected"><WorkflowHeading eyebrow="Réception" title="Nouvelle demande détectée" /><dl className="prospect-details">{incomingDetails.map(([label, value], index) => <div className={index < revealed ? "is-visible" : ""} key={label}><dt>{label}</dt><dd>{value}</dd></div>)}</dl><ActivityLine active>{revealed < 4 ? "Informations récupérées…" : "Analyse du flux…"}</ActivityLine></div>;
}
function Analysis({ revealed }: { revealed: number }) {
  const progress = Math.min(100, Math.max(0, (revealed - 1) * 34));
  return <div className="workflow-body" key="qualified"><WorkflowHeading eyebrow="Analyse" title="Contexte préparé" /><ul className="workflow-checklist">{analysisSteps.map((step, index) => <WorkflowStep key={step} label={step} status={index < revealed ? "complete" : index === revealed ? "active" : "pending"} />)}</ul><div className="qualification-result"><div><span>Avancement</span><strong>{progress}<small> / 100</small></strong></div><div className="priority"><span>Suite</span><strong>Préparée</strong></div></div><div className="score-track" aria-hidden="true"><span style={{ width: `${progress}%` }} /></div></div>;
}
function TriggeredActions({ revealed }: { revealed: number }) {
  return <div className="workflow-body" key="actions"><WorkflowHeading eyebrow="Exécution" title="Actions préparées" /><ul className="workflow-checklist workflow-checklist--actions">{actionSteps.map((step, index) => <WorkflowStep key={step} label={step} status={index < revealed ? "complete" : index === revealed ? "active" : "pending"} />)}</ul></div>;
}
function CompletedWorkflow() {
  return <div className="workflow-body workflow-complete" key="complete"><WorkflowHeading eyebrow="Terminé" title="Flux préparé" /><div className="completion-summary"><div><span>Données</span><strong>Prêtes</strong></div><div><span>Action</span><strong>Préparée</strong></div><div><span>Équipe</span><strong>Informée</strong></div></div><div className="workflow-duration"><span aria-hidden="true">✓</span> Le contexte est prêt pour <strong>la prochaine décision</strong></div></div>;
}
